...
public final class FluidloggedCompat {
   ...
   public static boolean isLoaded() {
      return FluidloggedCompat.Present.VALUE;
   }

   public static boolean hasFluid(IBlockAccess world, BlockPos pos) {
      return FluidloggedCompat.Present.VALUE && FluidloggedCompat.Hooks.hasFluid(world, pos);
   }

   public static void logWater(World world, BlockPos pos, IBlockState here) {
      if (FluidloggedCompat.Present.VALUE) {
         FluidloggedCompat.Hooks.logWater(world, pos, here);
      }
   }
   ...
   private static final class Hooks {
      ...
      private static void logWater(World world, BlockPos pos, IBlockState here) {
         FluidloggedUtils.setFluidState(world, pos, here, FluidState.of(FluidRegistry.WATER), false, 2);
      }
   }

   private static final class Present {
      private static final boolean VALUE = Loader.isModLoaded("fluidlogged_api");
   }
}
